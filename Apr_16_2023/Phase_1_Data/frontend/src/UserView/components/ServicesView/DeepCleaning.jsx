...
const DeepCleaningConfirmation = ({
  ...
  const [editableAddress, setEditableAddress] = useState(address || ""); // Initialize address as an empty string
  const [calendarBlockIndex, setCalendarBlockIndex] = useState(null);

  // Check if service with svc_id 3 exists in the passed serviceDetails.
  const isServiceFound = serviceDetails?.some(
    (service) => service.svc_id === 3
  );
  ...
  return (
    <div
      className={`max-w-md mx-auto  p-4 sm:p-2 ${tailwindStyles.whiteCard} min-h[500px] max-h-[500px] rounded-lg shadow-lg relative`}
    >
      ...
      <div className="flex items-center justify-between px-1 py-2 rounded-t-lg flex-nowrap gap-1 sm:gap-3">
        ...
        <div className="flex items-center min-w-0 space-x-3">
          <div className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-lg shrink-0">
            <img
              src="/Package/Package_Image_4.png"
              alt="Deep Cleaning"
              className="w-7 h-7"
            />
          </div>
